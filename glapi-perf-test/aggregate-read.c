...
void dispatch_read_binary(){
    int i,status;

    for(i=0;i<sizeof(server_list)/sizeof(int);i++){
        if(!(pid_list[i] = fork())) {
            char command[512];
            sprintf(command, "scp ./read tstore%02d:/tmp/read", server_list[i]);
            system(command);
			exit(0);
        }
    }
...
void test_aggregate_performance(char *volname){
    ...
    for(i=0;i<sizeof(server_list)/sizeof(int);i++){
        if(!(pid_list[i] = fork())) {
            char command[512];
            sprintf(command, "ssh tstore%02d \'numactl --cpunodebind=0 --membind=0 /tmp/read %d %d %s\' ", server_list[i], i * TIMES, TIMES, volname);
            system(command);
			exit(0);
        }
    }
...
void performance_analysis(){
    ...
    for(i=0;i<sizeof(server_list)/sizeof(int);i++){
        if(!(pid_tmp[i] = fork())) {
            char command[512];
            sprintf(command, "scp tstore%02d:/tmp/read.time ./times/%d.time", i,i);
            system(command);
			exit(0);
        }
    }
