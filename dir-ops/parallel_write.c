...
int main(int argc, char* argv[]){
    ...
    */
    ...
    /*
    if(recursive_create_fn(root_dir, 1, dir_depth, dir_cnt, file_size) == -1){
    ...
    */
    ...
    for(int i = 0; i < numjobs; i++){
        if((pids[i] = fork()) == 0){
            char signature[1024];
            sprintf(signature, "p-%d", getpid());
            if(recursive_create_fn(root_dir, 1, dir_depth, dir_cnt, file_size, signature) == -1){
                sprintf(output_buf, "%s recursive mkdir/files %s error %s", signature, root_dir, strerror(errno));
                EXIT(output_buf);
            }
            exit(0);
        }
